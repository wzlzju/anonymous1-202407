...
def draw(g, layout=None, pos=None, draw_edges_of_g=None, ax=None, label=None, 
    ...
    if layout == "cycle":
        ...
        pos = pos
    elif layout == "ring_cluster":
        pos = nx.spring_layout(g, k=0.3, pos=nx.circular_layout(g, scale=0.5))
    elif layout == "default":
        pos = nx.spring_layout(g, pos=nx.circular_layout(g))
    else:
        print("Unsupported layout:", layout)
    ...
    kwargs["edge_color"] = [hsv2rgb(0.736, d[2]['weight']*(1-min_S)+min_S, 1.0) for d in draw_edges_of_g.edges.data()]
    if f_node_color:
    ...
    nx.draw(draw_edges_of_g, pos=pos, ax=ax, **kwargs)
    if not ax:
        plt.axis('equal')  
        plt.show()
...
def draw2(g, layout=None, pos=None, draw_edges_of_g=None, draw_nodes=None, ax=None, label=None, 
        f_node_color=None, f_edge_color=None, f_node_size=None, f_edge_width=None,
        node_color=None, edge_color=None, node_size=None, edge_width=None, alpha=None,
    ...
    if "*" in layout:
        ...
        post_layout = nx.spring_layout
    if layout == "cycle":
        pos = post_layout(g, pos=nx.circular_layout(g))
    elif layout == "multilayer":
        pos = post_layout(g, pos=nx.multipartite_layout(g, subset_key="layer"))
    elif layout == "random":
        pos = post_layout(g)
    elif layout == "given_init":
        pos = post_layout(g, pos=pos)
    ...
    # min_S = 0
    kwargs["node_size"] = 10
    kwargs["alpha"] = 0.6 if alpha is None else alpha
    if channel == 'S':
    ...
    if draw_nodes:
        nx.draw_networkx_nodes(draw_edges_of_g, pos=pos, ax=ax, nodelist=draw_nodes, node_color='#808080', node_size=kwargs["node_size"], alpha=kwargs["alpha"])
    if not ax:
        # plt.axis('equal')  
        # plt.show()
        pass
