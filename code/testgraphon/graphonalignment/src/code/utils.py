...
def list_files_in_directory(directory):  
    try:  
        # 使用 os.listdir 获取目录中的内容  
        directory_contents = os.listdir(directory)  
          
        # 过滤出所有的文件，忽略子目录  
        file_list = [file for file in directory_contents if os.path.isfile(os.path.join(directory, file))]  
...
def exist_clear_or_mkdir(folder_path):
  # print("exist: {}".format(os.path.exists(folder_path)))
  if not os.path.exists(folder_path):  
    os.mkdir(folder_path)
  else:
    clear_folder(folder_path)
    
def get_cluster_mapping(graphon):
    cluster_info = nx.get_node_attributes(graphon, 'cluster')
    cluster_mapping = {}
    for node_id, cluster_id in cluster_info.items():
        if cluster_id not in cluster_mapping:
            cluster_mapping[cluster_id] = [node_id]
        else:
            cluster_mapping[cluster_id].append(node_id)
