...
def getDegreeListsVertices(g,vertices,calcUntilLayer, weighted=False, edge_attributes=None, maxdegree=None):
    degreeList = {}

    for v in vertices:
        degreeList[v] = getDegreeLists(g,v,calcUntilLayer, weighted, edge_attributes, maxdegree)
...
def getDegreeLists(g, root, calcUntilLayer, weighted=False, edge_attributes=None, maxdegree=None):
    t0 = time()
    ...
    while queue:
        ...
        if weighted:
            l.append([edge_attributes[vertex,vj]['weight'] for vj in g[vertex]]+[0.0]*(maxdegree-len(g[vertex])))
        else:
...
def calc_distances_all(vertices,list_vertices,degreeList,part, compactDegree = False, weighted=False):
    ...
    for v1 in vertices:
        ...
        for v2 in list_vertices[cont]:
            ...
            for layer in range(0,max_layer):
                #t0 = time()
                if weighted:
                    dist, path = fastdtw(lists_v1[layer],lists_v2[layer],radius=1,dist=scipy.stats.wasserstein_distance)
                else:
                    dist, path = fastdtw(lists_v1[layer],lists_v2[layer],radius=1,dist=dist_func)
                #t1 = time()
...
def exec_bfs(G,workers,calcUntilLayer, weighted=False, edge_attributes=None, maxdegree=None):
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        part = 1
        for c in chunks:
            job = executor.submit(getDegreeListsVertices,G,c,calcUntilLayer, weighted, edge_attributes, maxdegree)
            futures[job] = part
            part += 1
