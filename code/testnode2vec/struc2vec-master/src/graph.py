...
def load_edgelist(file_, undirected=True):
    ...
    for l in f:
      if(len(l.strip().split()[:2]) > 1):
        ...
        if(len(l.strip().split()) > 2):
          attributesStr = ' '.join(l.strip().split()[2:])
          G.edge_attributes[x,y] = eval(attributesStr)
        if undirected:
          G[y].append(x)
          if(len(l.strip().split()) > 2):
            attributesStr = ' '.join(l.strip().split()[2:])
            G.edge_attributes[y,x] = eval(attributesStr)
      else:
        x = l.strip().split()[:2]
        x = int(x[0])
        G[x] = []  
  ...
  G.make_consistent()
  G.maxdegree = max([len(G[v]) for v in G.keys()])
  return G
