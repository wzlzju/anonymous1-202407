...
class Graph():
	def __init__(self, g, is_directed, workers, untilLayer = None, weighted=False):

		logging.info(" - Converting graph to dict...")
		self.G = g.gToDict()
		self.edge_attributes = g.edge_attributes
		self.maxdegree = g.maxdegree
	...
	def preprocess_neighbors_with_bfs(self):
		...
		with ProcessPoolExecutor(max_workers=self.workers) as executor:
			job = executor.submit(exec_bfs,self.G,self.workers,self.calcUntilLayer, self.weighted, self.edge_attributes, self.maxdegree)
	...
	def calc_distances_all_vertices(self,compactDegree = False):
		...
		with ProcessPoolExecutor(max_workers = self.workers) as executor:
			...
			for c in chunks:
				...
				job = executor.submit(calc_distances_all, c, list_v, degreeList,part, compactDegree = compactDegree, weighted=self.weighted)
				futures[job] = part
				part += 1
